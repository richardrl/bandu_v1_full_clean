...
class CVAESeg(nn.Module):
    def __init__(self,
        ...
        :param normalize:
        :param sample_prior_bool: whether or not to generate prior samples to be sent into the out dict
        """
        super().__init__()
        ...
        assert decoder_type in ["seg", "segncls"]
        ...
        self.gpu_0 = torch.device("cuda:0")
        self.gpu_1 = torch.device("cuda:1")
        ...
        assert label_type in ["btb", "canonical_pc"]
        ...
        if prior_type == "vamp" or prior_type == "vamp_weighted":
            ...
            assert prior_predictor_type in ["segncls", "clsncls", "cls"]
            ...
            if prior_predictor_type == "segncls":
                ...
                self.prior_predictor = PointnetCls(output_dim=num_mog_prior_components * self.latent_dim * 2 + num_mog_prior_components,
                                                   channel=6 if use_normals else 3,
                                                   normal_channel=use_normals)
            else:
    ...
    def sample_mog(self, weights, z_mu, z_logvar, num_components,
        ...
        if reparameterize:
            ...
            indices = torch.multinomial(probs, 1, replacement=True) \
                .reshape(nB, z_samples_per_sample).to(z_mu.device)
    ...
    def decode_batch(self, batch, z_samples_per_sample=1, **kwargs):
        ...
        prior_sampled_z = self.sample_prior(batch, z_samples_per_sample, **kwargs)
        ...
        pc_X = pc_X.unsqueeze(1).expand(-1, z_samples_per_sample, -1, -1)\
            .reshape(nB*z_samples_per_sample, num_points, -1)
        pc = torch.cat((pc_X,
                        prior_sampled_z.reshape(nB*z_samples_per_sample, num_points, self.latent_dim)), dim=-1).permute(0, 2, 1)
        ...
        return pre_sigmoid_logits.reshape(nB, z_samples_per_sample, num_points, self.pointcloud_decoder.output_dim)