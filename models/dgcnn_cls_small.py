...
class DGCNNClsSmall(nn.Module):
    def __init__(self, n_knn=20, num_class=40, normal_channel=False,
                 num_channels=3):
        super().__init__()
        self.n_knn = n_knn
        ...
        #                            self.bn4,
        #                            nn.LeakyReLU(negative_slope=0.2))
        self.conv4 = nn.Sequential(nn.Conv1d(256, 512, kernel_size=1, bias=False),
                                   self.bn4,
                                   nn.LeakyReLU(negative_slope=0.2))
